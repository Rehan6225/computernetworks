...
char buff[1000],buff1[1000]; pid_t cpid=0;
bzero(&servaddr,sizeof(servaddr));
servaddr.sin_family=AF_INET;
servaddr.sin_addr.s_addr=inet_addr(argv[1]);
servaddr.sin_port=htons(16000);
...
address*/ cd=connect(sd,(struct sockaddr*)&servaddr,sizeof(servaddr));
/*Fork is used to create a new
process*/ la:
cpid=fork();
if(cpid==0)
{
